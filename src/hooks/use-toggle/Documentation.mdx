...
import * as allStories from './use-toggle.stories';

<Meta of={allStories} />

# useToggle

`useToggle` is a hook that implements a common state pattern – it switches state between given values.
...
`useToggle` accepts a single parameter:

- `initialValue` - The initial boolean values. array of two values.

## Returns

`useToggle` returns an tuple with the following values:
